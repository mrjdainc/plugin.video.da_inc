...
from da_inc.util import notify, getLocalizedString
from da_inc.logger import log
from da_inc.config import da_inc_HOST
from da_inc.addon import ADDON, ADDON_ID
...
def register(search, search_movie, search_episode, search_season=None):
    ...
    try:
        ...
        try:
            ...
            notify("%s: %s" % (getLocalizedString(30224), repr(e).encode('utf-8')))
            try:
                urllib_request.urlopen("%s/provider/%s/failure" % (da_inc_HOST, ADDON_ID))
            except:
                pass
    finally:
        try:
