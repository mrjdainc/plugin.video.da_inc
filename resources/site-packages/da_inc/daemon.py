...
from da_inc.osarch import PLATFORM, get_platform
from da_inc.config import da_inc_HOST
from da_inc.addon import ADDON, ADDON_ID, ADDON_PATH
from da_inc.util import notify, dialog_ok, system_information, getLocalizedString, getWindowsShortPath
...
def get_da_inc_binary():
    global binary_platform
    binary_platform = get_platform()

    binary = "da_inc" + (binary_platform["os"] == "windows" and ".exe" or "")
    ...
    if not os.path.exists(dest_binary_path) or not os.path.exists(binary_path) or get_da_inc_checksum(dest_binary_path) != get_da_inc_checksum(binary_path) or not filecmp.cmp(dest_binary_path, binary_path, shallow=True):
        log.info("Updating da_inc daemon...")
        try:
...
def clear_fd_inherit_flags():
    # Ensure the spawned da_inc binary doesn't inherit open files from Kodi
...
def jsonrpc_enabled(notify=False):
    try:
        ...
        if notify:
            ...
            dialog.ok("da_inc", getLocalizedString(30199))
    return False

def start_da_inc(**kwargs):
    jsonrpc_failures = 0
    ...
    da_inc_dir, da_inc_binary = get_da_inc_binary()

    log.info("Binary dir: %s, item: %s " % (da_inc_dir, da_inc_binary))
    if da_inc_dir is False or da_inc_binary is False:
        return False
    ...
    args = [da_inc_binary]
    kwargs["cwd"] = da_inc_dir

    if binary_platform["os"] == "windows":
        args[0] = getWindowsShortPath(da_inc_binary)
        kwargs["cwd"] = getWindowsShortPath(da_inc_dir)
        ...
        env = os.environ.copy()
        env["LD_LIBRARY_PATH"] = "%s:%s" % (da_inc_dir, env.get("LD_LIBRARY_PATH", ""))
        kwargs["env"] = env
    ...
    log.info("da_inc: start args: %s, kw: %s" % (args, kwargs))
...
def shutdown():
    try:
        urllib_request.urlopen(da_inc_HOST + "/shutdown")
    except Exception:
        pass

def wait_for_abortRequested(proc, monitor):
    monitor.closing.wait()
    log.info("da_inc: exiting da_inc daemon")
    try:
        if proc is not None:
            proc.terminate()
    except OSError:
        pass  # Process already exited, nothing to terminate
    log.info("da_inc: da_inc daemon exited")

def da_inc_thread(monitor):
    crash_count = 0
    try:
        while not xbmc.abortRequested:
            log.info("da_inc: starting da_inc")
            proc = None
            if hasSubprocess:
                proc = start_da_inc(stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                if not proc:
                    break
            else:
                log.info("da_inc: current system is unable to run the binary")
                break
            ...
            if binary_platform["os"] == "windows":
                ...
                # read. We count on the fact that da_inc daemon flushes its log
