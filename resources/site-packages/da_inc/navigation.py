...
from da_inc.util import notify, getLocalizedString, getLocalizedLabel
...
def getInfoLabels():
    ...
    if not tmdb_id:
        ...
        if 'tmdb' in query and 'type' in query and query['type'][0] == 'search':
            ...
            url = "%s/search/infolabels/%s" % (da_inc_HOST, tmdb_id)
            ...
            tmdb_id = query['tmdb'][0]
            url = "%s/movie/%s/infolabels" % (da_inc_HOST, tmdb_id)
        elif 'show' in query:
            tmdb_id = query['show'][0]
            if 'season' in query and 'episode' in query:
                url = "%s/show/%s/season/%s/episode/%s/infolabels" % (da_inc_HOST, tmdb_id, query['season'][0], query['episode'][0])
            else:
                url = "%s/show/%s/infolabels" % (da_inc_HOST, tmdb_id)
        else:
            url = "%s/infolabels" % (da_inc_HOST)
    elif 'movie' in sys.argv[0]:
        url = "%s/movie/%s/infolabels" % (da_inc_HOST, tmdb_id)
    elif ('episode' in sys.argv[0] or 'show' in sys.argv[0]) and len(id_list) > 2:
        url = "%s/show/%s/season/%s/episode/%s/infolabels" % (da_inc_HOST, tmdb_id, id_list[1], id_list[2])
    elif 'show' in sys.argv[0] and len(id_list) == 2:
        url = "%s/show/%s/season/%s/episode/%s/infolabels" % (da_inc_HOST, tmdb_id, id_list[1], 1)
    else:
        url = "%s/infolabels" % (da_inc_HOST)
...
def _json(url):
    with closing(urllib_request.urlopen(url)) as response:
        if response.code == 300:
            ...
            # Pause currently playing da_inc file to avoid doubling requests
            try:
            ...
            # if "da_inc" in response.geturl().lower() and "http" not in response.geturl().lower():
...
def run(url_suffix="", retry=0):
    ...
    url = sys.argv[0].replace("plugin://%s" % ADDON_ID, da_inc_HOST + url_suffix) + sys.argv[2]
    query_add = ""
